...
- 当文档语言为德语（`lang=de`）时自动载入 `babel` 宏包（`german`），由 `babel` 接管 `\languagename` 与德语断词规则；其他语言不载入 `babel`。
- 用户在其他语言文档中手动载入 `babel` 时，文档类同样会检测并兼容，将语言状态交给 `babel` 管理（文档开头会切换到主语言，使 `babel` 语言状态与主语言对齐；未在 `babel` 中注册的语言保持 `babel` 当前语言不变，不报错）。

### Changed

- `\SetInfoEntry` 的敏感字段标记从键名后缀 `*` 移至显示名称开头（如 `author = * 姓名`），
  并以显示名称开头的 `!` 解除敏感状态（如 `author = ! 姓名`）；
  重定义已存在的字段仅更新显示名称或敏感状态，不再误报忽略警告。
...
[Unreleased]: https://github.com/sjtug/SJTUTeX/compare/v2.3.1...HEAD
[v2.3.1]:     https://github.com/sjtug/SJTUTeX/compare/v2.3...v2.3.1
